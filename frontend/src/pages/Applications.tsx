...
const createApplicationSchema = z.object({
    ...
    details: z.string().min(10, "详情至少10个字符").max(1000, "详情不能超过1000个字符"),
    applied_credits: z.number().min(0.5, "申请学分至少0.5").max(10, "申请学分最多10"),
});

const reviewApplicationSchema = z.object({
    status: z.enum(['approved', 'rejected']),
    review_comment: z.string().min(1, "请填写审核意见").max(500, "审核意见不能超过500个字符"),
    approved_credits: z.number().min(0, "批准学分不能为负数").max(10, "批准学分最多10"),
});
...
export default function ApplicationsPage() {
    ...
    const filteredApplications = applications.filter(app => {
        const matchesSearch = app.student_name?.includes(searchTerm) || 
        ...
        <div className="space-y-8 p-4 md:p-8">
            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                ...
                {canCreateApplication && (
                    <Button onClick={() => setCreateDialogOpen(true)} className="rounded-lg shadow transition-all duration-200 hover:scale-105">
            ...
            <Card className="rounded-xl shadow-lg">
                ...
                <CardContent>
                    <div className="flex flex-col md:flex-row gap-4 items-center">
                        ...
                        <Button variant="outline" onClick={fetchApplications} disabled={loading} className="rounded-lg shadow">
                    ...
                    <div className="border rounded-xl overflow-x-auto">
                        <Table>
                            <TableHeader className="bg-muted/60">
                                <TableRow>
                                    <TableHead className="font-bold text-primary">申请ID</TableHead>
                                ...
                                {loading ? (
                                    ...
                                    filteredApplications.map(app => (
                                        <TableRow key={app.id} className="hover:bg-muted/40 transition-colors">
                                            <TableCell className="font-semibold text-primary">#{app.id}</TableCell>
                                            <TableCell>
                                                <div className="font-medium">{app.student_number}</div>
                                                <div className="text-xs text-muted-foreground">{app.student_name}</div>
                                            </TableCell>
                                            <TableCell>
                                            ...
                                            <TableCell>
                                                <span className="font-bold text-blue-600">{app.applied_credits}</span>
                                            </TableCell>
                                            <TableCell>
                                                <Badge
                                                    className={
                                                ...
                                                >
                                                    {app.status === 'unsubmitted' ? <Clock className="w-3 h-3 mr-1 inline" /> : null}
                                                    {app.status === 'pending' ? <Clock className="w-3 h-3 mr-1 inline" /> : null}
                                                    {app.status === 'approved' ? <CheckCircle className="w-3 h-3 mr-1 inline" /> : null}
                                                    {app.status === 'rejected' ? <XCircle className="w-3 h-3 mr-1 inline" /> : null}
                                                    {app.status === 'unsubmitted' ? 'Unsubmitted' : app.status === 'pending' ? 'Pending' : app.status === 'approved' ? 'Approved' : 'Rejected'}
                                                </Badge>
                                            </TableCell>
            ...
            <Dialog open={isCreateDialogOpen} onOpenChange={setCreateDialogOpen}>
                <DialogContent className="max-w-2xl">
                    ...
                    <Form {...createForm}>
                        <form onSubmit={createForm.handleSubmit(handleCreateApplication)} className="space-y-6">
                            <FormField
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">{selectedApp.applied_credits}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium">批准学分</label>
                                    <p className="text-sm text-muted-foreground">{selectedApp.approved_credits}</p>
                                </div>
                            </div>
                            <div>
            ...
            <Dialog open={isReviewDialogOpen} onOpenChange={setReviewDialogOpen}>
                <DialogContent>
                    ...
                    <Form {...reviewForm}>
                        <form onSubmit={reviewForm.handleSubmit(handleReviewApplication)} className="space-y-4">
                            <FormField
