...
func RegisterRouters(userHandler *handlers.UserHandler) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		{
			// 根据配置生成 departments 树（学校 / 学部 / 专业 / 班级）
			testData.POST("/departments-from-options", userHandler.GenerateDepartmentsFromOptions)
		}
		...
		{
			// 自助学生注册（无需认证）：POST /api/students/register
			...
			auth.Use(authMiddleware.AuthRequired())
			{
				...
				admin.Use(permissionMiddleware.AdminOnly())
				{
					admin.POST("", userHandler.CreateStudent)   // 管理员创建学生
		...
		search := api.Group("/search")
		{
			...
			auth.Use(authMiddleware.AuthRequired())
			{
				...
				allUsers.Use(permissionMiddleware.AllUsers())
				{
					allUsers.GET("/users", userHandler.SearchUsers) // 搜索用户
				}
			}
		}
	}
